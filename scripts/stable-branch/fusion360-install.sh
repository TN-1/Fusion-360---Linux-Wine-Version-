...
function check-requirement {
    ...
    if [ "$answer" != "${answer#[YyJj]}" ]; then
        ...
        wmctrl -r ':ACTIVE:' -b toggle,fullscreen && # Does something?
        echo "No Error!" # This is in place to allow the script to continue
...
# Does Fusion 360 already exist on the system?  
...
# Autodesk Fusion 360 will be installed using Wine and Winetricks
...
# Autodesk Fusion 360 will be installed using Wine and Winetricks (Custom path)
...
# New install - Confirm user wants to continue
function welcome-screen-1 {
    HEIGHT=15
    WIDTH=60
    CHOICE_HEIGHT=2
    BACKTITLE="$text_2" # Version number
    TITLE="$text_2_1" # Prompt
    MENU="$text_2_2" # Prompt

    OPTIONS=(1 "$text_2_3" # Yes
            2 "$text_2_4") # No
    ...
    case $CHOICE in
            1)
                ...
                driver_used=0 &&
                select-dxvk-or-opengl # Proceed to driver selection
                ;;
            2)
                exit
                ;;
...
function welcome-screen-2 {
    ...
    WIDTH=180
    CHOICE_HEIGHT=3
    BACKTITLE="$text_3" # Version number
    TITLE="$text_3_1" # Prompt
    MENU="$text_3_2" # Prompt 

    OPTIONS=(1 "$text_3_3" # Reinstall
            2 "$text_3_4" # Update or repair
            3 "$text_3_5") # Uninstall
...
# Driver selection (DXVK or OpenGL) - Part 1
